			...
			if (Cw_BakRcv2 <= RCV2_MAX) // 接收长度正确,继续处理.
			{
				...
				else if (F_GprsTransToCom)
				{
					if (Cw_BakRcv2 <= RCV2_MAX) // 接收长度正确,继续处理.
					{
						...
						//---------------------------------------------------------------
						// 4. 没有连接时，文本显示器跟GP311通讯
						//	<32 是03,06,16指令的范围内,没有连接时S_M35<0x1B

						// ZCL 2016.7.5 取消掉与文本通讯功能。否则导致GP311重启时，接收到SM510的主动发送，GP311回发，SM510收到，再次发送的死循环。

						/* 				else if(Rcv2Buffer[0]==2 && Rcv2Buffer[1]<32 && S_M35<0x1B)
										{
											B_RunGprsModule=0;		//正在与文本通讯，清此标志。上电4秒内没有文本，赋值1，才运行GPRS
											C_DelayRunGprsModule=0;		//正在与文本通讯，请计数器

											j=CRC16( Rcv2Buffer, Cw_BakRcv2-2);		// CRC 校验
											k=j>>8;
											s=j;
											if ( k==Rcv2Buffer[Cw_BakRcv2-2]
												&& s==Rcv2Buffer[Cw_BakRcv2-1] )
											{											// CRC校验正确
												if ( Rcv2Buffer[1]==3 )		// 03读取保持寄存器
												{
													B_Com1Cmd03=1;
													j=Rcv2Buffer[2];
													w_Com1RegAddr=(j<<8)+Rcv2Buffer[3];
												}
												else if ( Rcv2Buffer[1]==16 )	// 16预置多寄存器
												{
													C_ForceSavPar=0;		// 强制保存参数计数器=0
		...
		/*
				...
				if (j)
				{
					...
					if (Lw_Com2RegAddr < STM32_RPAR_ADDRESS)
					{
						...
						Lw_Com2RegAddr = Lw_Com2RegAddr - STM32_WRPAR_ADDRESS;
					}
