...
void LoRa_MasterSend(void) // LoRa主发送程序
{
	...
	/* 	Pw_LoRaModule1Add=112;		//ZCL 2022.5.19
		Pw_LoRaModule2Add=113;

		Pw_LoRaModule3Add=114;
		Pw_LoRaModule4Add=115;
	...
	// if ( w_LoRaLeiDaModuleAdd )	i++;					//ZCL 2020.9.30 一个专门的采集DI的 SZM220
		...
		if (B_LoRaSendMaster == 1) //&& B_LoRaRCVCommandOk
		{
			if (T_LoRaDelaySend != SClk10Ms)
			{
				...
				if (C_LoRaDelaySend > 20) // 210可以 ZCL 2019.10.17	；使用连续接收模式后，2都可以了！ 解决！
				{
					...
					{
						if (w_ZhuanFaAdd < 40)				  // w_ZhuanFaAdd >= 0 &&
						...
						if (w_ZhuanFaAdd >= 200 && w_ZhuanFaAdd < 240)
							LoRaTxBuf[0] = Pw_LoRaModule6Add; // 泵6			YLS 2023.10.24修改地址错误
