...
{
	if(Data & 0x01)		LCD12864_DATA0_H;
	else				LCD12864_DATA0_L;

	if(Data & 0x02)  	  	LCD12864_DATA1_H;
	else			  	LCD12864_DATA1_L;

	if(Data & 0x04)  	  	LCD12864_DATA2_H;
	else			  	LCD12864_DATA2_L;

	if(Data & 0x08)  	  	LCD12864_DATA3_H;
	else			  	LCD12864_DATA3_L;

	if(Data & 0x10)  	  	LCD12864_DATA4_H;
	else			  	LCD12864_DATA4_L;

	if(Data & 0x20)  	  	LCD12864_DATA5_H;
	else			  	LCD12864_DATA5_L;

	if(Data & 0x40)  	  	LCD12864_DATA6_H;
	else			  	LCD12864_DATA6_L;

	if(Data & 0x80)  	  	LCD12864_DATA7_H;
	else			  	LCD12864_DATA7_L;
...
u8 LCD12864_ReadOneByte(void)
{
	...
	if(LCD12864_RDATA0)		RData |= 0x01;
	if(LCD12864_RDATA1)		RData |= 0x02;
	if(LCD12864_RDATA2)		RData |= 0x04;
	if(LCD12864_RDATA3)		RData |= 0x08;
	if(LCD12864_RDATA4)		RData |= 0x10;
	if(LCD12864_RDATA5)		RData |= 0x20;
	if(LCD12864_RDATA6)		RData |= 0x40;
	if(LCD12864_RDATA7)		RData |= 0x80;
...
//	int BitLength=0;  			//要显示的数字的位数
	...
	if(myLen == 0)	return;				//数据长度是0.

	for(i = 0; i < 32; i++)	Wei[i] = 0;	 	//
...
/* 		if(S_ModParStatus	)
		...
		else if(Lw_PageNo== LatLonPage  || Lw_PageNo== LatLonPage2 )		//ZCL 2018.10.28
		{
			LCD12864_WriteCommand(Address);
			// if(mod)
				// LCD12864_WriteData('>');			
...
/* 	else if(Num == 0) 					  	//要显示的数字是0.
	{
		...
		for(i = 0; i < AllLen; i++)		//可以指定显示几个0 ZCL
		{
			LCD12864_WriteData('0');
		}
	} */
