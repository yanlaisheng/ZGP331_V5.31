...
void Com1_Port_OutByte(u8 sendchar)
{
	...
	while (USART_GetFlagStatus(USART1, USART_FLAG_TXE) == RESET)
		;										  //==0正在发送，等待！
	USART_ClearITPendingBit(USART1, USART_IT_TC); // 2013.9.2	 清除USART_IT_TC标志
	USART_SendData(USART1, tmp_Com1);			  // 发送数据

	// 2013.9.4 重点说明，下面一句用寄存器方式好用，用库方式竟然不行，存在BUG
	// 库问题是明明 USART1->SR 06位TC已经为1，但USART_GetFlagStatus(USART1, USART_IT_TC) 检测不到
	/* Loop until the end of transmission */
	// while (USART_GetFlagStatus(USART1, USART_IT_TC) == RESET);		//这句不行，存在BUG  ZCL 2013.9.4
...
u8 Com1_Port_InByte(u16 Time)
{
	...
	ch = 0;
	C_ReceiveChar_Com1 = 0; // 接收字符时间计数器
	LastError_Com1 = 0;

	while (C_ReceiveChar_Com1 < Time) // C_ReceiveChar在SysTickHandler中每毫秒++
	{
...
void XmodemFileTransfer_Com1(void) // ZCL 2013.5.29 	Xmodem协议进行文件传输 （文件传输：File Transfer ）
{
...
int XmodemReceive_Com1(uchar *dest, int destsz) // xmodem协议接收文件
{
	...
	for (;;)
	{
		for (retry = 0; retry < 16; ++retry) //? 16 所有的超时及错误事件至少重试10次
		{
			if (checkchar)
			{
				...
				Com1_Port_OutByte(checkchar); // 发送：校验模式字符
			}
			c = Com1_Port_InByte((DLY_1S) << 1); // 接收：在2S时间内接收到的字符
			if (LastError_Com1 == 0)
			{
				switch (c)
				{
				...
				case EOT: // EOT:结束
					// Flushinput();
					Com1_Port_OutByte(ACK); // 接收方用 ACK 进行确认
					//*(dest+len)=0;
	...
	start_recv:
		...
		*p++ = c;										  // 保存第一个字符
		...
		{
			c = Com1_Port_InByte(DLY_1S); // 字符间1秒延时
			Lw_GprsReceiveCount_Com1 = i;

			if (LastError_Com1 != 0) // 有错误跳出，发送NAK
				goto reject;
			*p++ = c; // 没有错误保存字符
		}
...
** 其他说明 : 程序按照短信设置、查询程序
********************************************************************************/
void XmodemRcvDoWith_Com1(void) // xmodem协议接收的数据进行处理			//ZCL 2013.5.30
{
	uchar i, r, s, err = 0;
	uint ip[4], w1, n = 0;
	// char *ptr;
	...
	while (1)
	{
		...
		if (ptr != NULL)
		{
			if (Lw_GprsTmp_Com1 == 0) // 打印个换行符，好分辨信息
				Com1_printf("\r\n-------------------Result info\r\n");
			...
			Xmodem_SetBaudItemLack_END_Com1: // 设置波特率中设置项缺少错误。GPW+0D,9600,8,1,2,3;
				if (w1 == 255 && err == 0)
				{
					// 错误参数的处理	//设置串行口项缺少错误
					err = 9;
