...
int main(void)
{
	...
	// printf("\r\nZCL123 ZCL123 ZCL123  \r\n");		//ZCL 2019.4.19 测试，可以发送
	...
	/* 	// Pw_LoRaSet=3;
		...
		//LoRa主机和从机
		F_LoRaEn=1;					//LoRa打开
		//F_LoRaToCom=1;			//LORA输出到COM2 (透传使用)


		//LoRa主机 根据需求设置
		F_GprsEn=1;									//GPRS打开。主机需要GPRS；GPRS数据自己需要
																	//（顺带可以串口2输出，GP311是透传转到串口）
			//F_GprsTransToCom=1;					//透传到COM
			//F_GprsMasterNotToCom=1;			//主机模式GPRS，不输出信息到串口（默认）
		F_GprsMasterToCom=1;				//主机模式GPRS，输出信息到串口（方便观察）
			// F_GpsToCom=1;
		//以上即为：Pw_LoRaSet=1;			Pw_GprsSet=9;		2019.4.2


		//LoRa主机
		Pw_LoRaMasterSlaveSel=1;			//LoRa主机
		Pw_LoRaEquipmentNo=2;					//LoRa 本机地址
		Pw_LoRaModule1Add=3;					//LoRa 从机1地址
		Pw_LoRaModule2Add=0;					//LoRa 从机2地址
		Pw_LoRaModule3Add=0;					//LoRa 从机3地址
		Pw_LoRaModule4Add=0;					//LoRa 从机4地址


		//LoRa从机
		// Pw_LoRaMasterSlaveSel=0;				//LoRa从机
		// Pw_LoRaEquipmentNo=3;					//LoRa 本机地址
		// Pw_LoRaModule1Add=0;						//LoRa 从机1地址
		// Pw_LoRaModule2Add=0;						//LoRa 从机2地址
		// Pw_LoRaModule3Add=0;						//LoRa 从机3地址
		// Pw_LoRaModule4Add=0;						//LoRa 从机4地址
		// B_LoRaRcv=0;										//
		// F_GprsEn=0;								//GPRS打开。主机需要GPRS；GPRS数据自己需要
																	// //（顺带可以串口2输出，GP311是透传转到串口）
		//F_GprsTransToCom=0;					//透传到COM
		//F_GprsMasterNotToCom=0;			//主机模式GPRS，不输出信息到串口（默认）
		//F_GprsMasterToCom=0;				//主机模式GPRS，输出信息到串口（方便观察）
		 */

	// ZCL 2019.11.29 GPS配置；下面直接赋值在测试时用
	//  Pw_LoRaSet=0;
	//  Pw_GprsSet=0;
	//  F_GpsToCom=1; 		//即设置Pw_GprsSet为16；
