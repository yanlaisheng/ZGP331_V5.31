...
void Menu_LoRaSetPar6(void) // 画面 LoRa设定参数
{
	if (Lw_SavePageNo != Lw_PageNo)
	{
		...
		// LCD12864_ClrText();          //清屏
		// 第一行
		LCD12864_String(0x80, "控制台信息      ");
		LCD12864_String(0x87, "  ");
		// 第二行
		LCD12864_String(0x90, "重启间隔      ");
		LCD12864_String(0x97, "分");
		// 第三行
		LCD12864_String(0x88, "空中速率      ");
		LCD12864_String(0x8F, "kb");
		// 第四行
		LCD12864_String(0x98, "控制台信息      ");
		LCD12864_String(0x9F, "  ");
	}

	ModParNum(2);
	// 显示和修改参数
	DispMod_Par(0x84, 0x05, 0x00, &Pw_ConsoleInfo, 4, F_ModPar4);
	DispMod_Par(0x94, 0x05, 0x00, &Pw_RebootInterval, 4, F_ModPar1);
	// DispMod_Par(0x8C, 0x04, 0x02, &w_LoRaDateRate, 5, F_ModPar3); // 类型4和5，在DispMod_Par()中没有用 ZCL 2018.8.3
...
void Menu_LoRaSetPar7(void) // 画面 LoRa设定参数
{
	...
	ModParNum(0);
	// 显示和修改参数
	DispMod_Par(0x84, 0x05, 0x00, &w_ScrVERSION, 5, 0);
	DispMod_Par(0x94, 0x04, 0x02, &w_ScrWriteYear, 1, 0); // ZCL 2019.4.3
