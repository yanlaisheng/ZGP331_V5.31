...
uc16 w_GprsParBootLst[50] = {
	60000, 0, 2, 1, 3,	   // 0-4		0=重启间隔时间 分钟,	1=,	2=Pw_GprsSetP
						   //			3=主机从机选择,4=LORA模块1地址
	4, 0, 0, 0, 0,		   // 5-9		5=,6=
						   //			7=LORA模块4地址 8=故障重启次数	9=修改参数
	0, 2, 0, 0, 0,		   // 10-14	10=参数初始化 11= 12=
						   //			13=出水口压力传感器初值 14=
	0, 1, 9, 0, 0,		   // 15-19	15= 16=LoRa 设定 17=
						   //			18= 19=
	6, 0, 0, 0, 0,		   // 20-24	20= 21= 22=
						   //			23=参数初始化 24=
	...
	4, 128, 1000, 1000, 0, // 45-49	45=HopPeriod 46=Pw_LoRaSetPayLoadLength  47=Pw_LoRaSetTxPacketTimeOut
						   //			48=Pw_LoRaSetRxPacketTimeOut 49=
						   // ZCL 2019.9.24  47、48  加上值
...
void Variable_Init(void) //	变量初始化
{
	...
	w_ScrVERSION = 526; // 软件版本号 VERSION	// 2017.9.13 -3 2018.7.17

	w_ScrWriteYear = 2024; // 程序编写年
	w_ScrWriteDate = 925;  // 程序编写日期
...
void ParLst_Init(void) // RAM中参数表列 初始化 (读出)
{
	...
	w_GprsSaveBaudRate3 = Pw_GprsBaudRate3;
...
void Boot_ParLst(void) // 初始化设定参数
{
	...
	// ZCL 2018.12.22
	else if (Pw_GprsBaudRate3 > 57600 && Pw_GprsBaudRate2 > 57600 && S_BootParLst == 0)
	{
		i = 4;
		B_ForceSavPar = 1; // 2013.10.1
	}
...
void DoWith(void) // 一些数据,记录的处理
{
	...
	// 波特率的修改  ZCL  2009.12.4
	/* 	if ( Pw_GprsBaudRate3!=w_GprsSaveBaudRate3 )
		{
			w_GprsSaveBaudRate3=Pw_GprsBaudRate3;
			Com0_config();
		}		 */
...
{
	// 波特率限制
	/* 	if ( Pw_GprsBaudRate3!=2400 && Pw_GprsBaudRate3!=4800 && Pw_GprsBaudRate3!=9600 && Pw_GprsBaudRate3!=19200
		&& Pw_GprsBaudRate3!=38400 && Pw_GprsBaudRate3!=57600 )
		{
			Pw_GprsBaudRate3=57600;								// 串口0波特率
		} */
