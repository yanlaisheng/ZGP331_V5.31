...
void Port_OutByte(u8 sendchar)
{
	...
	while (USART_GetFlagStatus(USART2, USART_FLAG_TXE) == RESET);			//==0正在发送，等待！
...
uchar CheckSPACEInput(void)
{
	...
	if(i==SPACE)
	{
		...
		{
			u1_printf("\r\n------------------------------------Begin \r\n");						
			u1_printf("Enter Xmodem set gprs par mode\r\n");			
...
void XmodemFileTransfer(void)					//ZCL 2013.5.29 	Xmodem协议进行文件传输 （文件传输：File Transfer ）
{
	...
	if( CheckSPACEInput() )
	{
		...
		u1_printf("Exit Xmodem , Enter normal run mode. \r\n");		
		u1_printf("-------------------------------------End \r\n");				
		i=1;
	}
...
start_recv:
		...
		*p++ = c;									//保存第一个字符
		...
		//上面接收完一帧数据
		//下面进行序号，序号补码，校验
		if ( xbuff[1] == (uchar)(~xbuff[2]) 
			&& 	( xbuff[1] == packetno || xbuff[1] == (uchar)packetno-1) 
		...
		if( ptr!=NULL )
		{
					...
					|| CommandNo==0x62 )
			{
				...
				for(s=0;s<4;s++)
				{
					...
					if(s==0 && ip[s]==0)
					{	
						ip[1]=0;ip[2]=0;ip[3]=0;s=3;		//2013.6.4 解决IP设置为空时显示出错问题
					}
					if(s==3)
						break;	//最后面没有'.'了
					//查找第一二三个'.' 
				...
				for(s=0;s<4;s++)
				{
					...
					else
					{
						//错误处理 标记：err=2
						err=2;
						u1_printf("IP Address Set Error!\r\n");
			...
			else if(CommandNo==0x01 || CommandNo==0x05 || CommandNo==0x06 
					|| CommandNo==0x21 || CommandNo==0x31 || CommandNo==0x41 || CommandNo==0x51 )
			{
				...
				if(err==3)
				{
					//填充参数错误的短信内容，准备发送短信				
					//错误参数的处理
					u1_printf("Par Item 0x%x: String Length Overlong Error! \r\n", CommandNo);						
				}
				else
				{
					//r(0x0D=回车 位置)  i(=位置)
			...
			else if(CommandNo==0x0D )
			{
				...
				else
				{
					//错误参数的处理
					err=4;
					u1_printf("Com1 Baudrate Set Error!\r\n");			
				}
				...
				else
				{
					//错误参数的处理
					err=5;
					u1_printf("Com1 DataBitLength Set Error!\r\n");			
				}				
				...
				else
				{
					//错误参数的处理
					err=6;
					u1_printf("Com1 StopBitLength Set Error!\r\n");			
				}						
				...
				else
				{
					//错误参数的处理
					err=7;
					u1_printf("Com1 ParityBitLength Set Error!\r\n");			
				}		
				...
				else
				{
					//错误参数的处理
					err=8;
					u1_printf("Com1 FlowControl Set Error!\r\n");			
				}		
...
Xmodem_SetBaudItemLack_END:		//设置波特率中设置项缺少错误。GPW+0D,9600,8,1,2,3;
				if(w1==255 && err==0)
				{
					//错误参数的处理	//设置串行口项缺少错误
					err=9;	
					u1_printf("Serial Port Item Lack Error!\r\n");						
				}				
			...
			if(err)
			{
				u1_printf("Xmodem File Transfer err= %d  !!!\r\n", err);
				u1_printf("Please restart File Transfer !!!\r\n");				
				break;			
			}
			else
				Lw_GprsTmp++;
		...
		{
			u1_printf("Xmodem set end，set par item %d times! OK\r\n", Lw_GprsTmp);
			break;			
		}
