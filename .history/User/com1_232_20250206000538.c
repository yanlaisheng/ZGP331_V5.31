...
void Com1_config(void)
{
	...
	{
		Pw_BaudRate1 = 9600;
		USART_InitStructure.USART_BaudRate = Pw_BaudRate1; // 9600;
	}
						...
						{							// CRC校验正确
							if (Rcv1Buffer[1] == 3) // 03读取保持寄存器
							{
								...
								if (Lw_Reg < STM32_RPAR_ADDRESS)
								{
									...
									p_wRead += Lw_Reg - STM32_WRPAR_ADDRESS;
								}
...
void Com1_SlaveSend(void) // 串口1从机发送
{
	...
	if (Pw_EquipmentType == 0) // 双驱泵
	{
		if (B_Com1Cmd03) // 读取保持寄存器
		{
			...
			// {
			// 	p_wRead = w_DNBParLst; // 设定STM32 PAR区
			// 	Lw_Com1RegAddr = Lw_Com1RegAddr - DNB_ADDRESS;
			// }
			...
			// }
			// // ZCL 2019.11.22 加入 w_GprsParLst PAR区
			// else if (Lw_Com1RegAddr < STM32_WRPAR_ADDRESS)
			// {
			// 	p_wRead = w_GprsParLst; // w_GprsParLst PAR区
			// 	Lw_Com1RegAddr = Lw_Com1RegAddr - SCR_WRPAR_ADDRESS;
			// }
			// // YLS 2023.12.14 修改 Pw_ParLst PAR区
			// else
			// {
			// 	p_wRead = Pw_ParLst;								   // 设定STM32 PAR区   Pw_ParLst
			// 	Lw_Com1RegAddr = Lw_Com1RegAddr - STM32_WRPAR_ADDRESS; // 50000
			// }
	...
	else // 变频电机
	{
		...
		if (B_Com1Cmd03) // 读取保持寄存器
		{
			...
			{
				p_wRead = Pw_ParLst; // 设定STM32 PAR区
				Lw_Com1RegAddr = Lw_Com1RegAddr - STM32_WRPAR_ADDRESS;
			}
			else if (Lw_Com1RegAddr < DSP_WRPAR_ADDRESS)
			{
				p_wRead = w_ParLst; // 读STM32 PAR区
	...
	if (i > 0) // 必须有子模块或者变频器，流量计才能进入发送程序
	{
		if (F_Com1SendNext) // 1. 控制发送步骤
		{
			...
			if (j == 1) // 发往子模块
			{
				...
				// USART_SendData(USART1, Txd1Buffer[Txd1Counter++]); // ZCL 2019.10.6
				USART_SendData(USART1, Txd1Buffer[Cw_Txd1++]);
				USART_ITConfig(USART1, USART_IT_TC, ENABLE); // 开始发送.
			}

			// ZCL 2022.8.24  16指令
			// YWZ320 中：#define	w_ComYaLiDecValue			w_ParLst[161]	// ZCL 2022.8.24 通讯压力值
			//			if (j) // 发往子模块  给压力表写压力
			//			{
			//				// Txd1Buffer[0]=Pw_EquipmentNo+S_Com1MSend;
			//				Txd1Buffer[1] = 16;	  // 功能码
			//				Txd1Buffer[2] = 0x00; // 开始地址（高位）
			//				Txd1Buffer[3] = 161;  // 开始地址（低位）  地址：9-23 设定参数查询
			//				Txd1Buffer[4] = 0;	  // 读取个数（高位）
			//				Txd1Buffer[5] = 1;	  // 读取个数（低位）
			//				Txd1Buffer[6] = 2;	  // 字节数

			//				if (j == 1)
			//				{
			//					Txd1Buffer[7] = w_OutPDec >> 8; // 预置数据（高位）
			//					Txd1Buffer[8] = w_OutPDec;		// 预置数据（低位）
			//				}
			//				else if (j == 2)
			//				{
			//					Txd1Buffer[7] = w_InPDec >> 8; // 预置数据（高位）
			//					Txd1Buffer[8] = w_InPDec;	   // 预置数据（低位）
			//				}

			//				w_Txd1ChkSum = CRC16(Txd1Buffer, 9);
			//				Txd1Buffer[9] = w_Txd1ChkSum >> 8; // /256
			//				Txd1Buffer[10] = w_Txd1ChkSum;	   // 低位字节
			////				Txd1Max = 11;
			////				Txd1Counter = 0;
			//				Cw_Txd1Max = 11;
			//				Cw_Txd1 = 0;
...
u16 Address(u16 *p, u16 Area)
{
	...
	{
		if (Area == 0)
			Lw_Reg = Position(Pw_ParLst, p) + DNB_ADDRESS; // STM32_WRPAR_ADDRESS 2023.12.16 YLS STM32_WRPAR_ADDRESS这个常量改为50000了
		else if (Area == 1)
	...
	{
		if (Area == 0)
			Lw_Reg = Position(Pw_ParLst, p) + STM32_WRPAR_ADDRESS;
		else if (Area == 1)
			Lw_Reg = Position(w_ParLst, p) + STM32_RPAR_ADDRESS;
		else if (Area == 2)
			Lw_Reg = Position(Pw_dspParLst, p) + DSP_WRPAR_ADDRESS;
...
u16 *AddressConvert(u16 L_Reg)
{
	...
	// ZCL 2019.11.22 加入 w_GprsParLst PAR区
	else if (L_Reg >= SCR_WRPAR_ADDRESS && L_Reg < STM32_WRPAR_ADDRESS)
	{
		p = w_GprsParLst; // w_GprsParLst PAR区
		p += L_Reg - SCR_WRPAR_ADDRESS;
	}
	...
	// YLS 2023.12.15
	else if (L_Reg >= STM32_WRPAR_ADDRESS)
	{
		p = Pw_ParLst; // Pw_ParLst PAR区
		p += L_Reg - STM32_WRPAR_ADDRESS;
	}
	return p;
}
