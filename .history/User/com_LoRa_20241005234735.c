...
void LoRa_SlaveSend(void) // LoRa从机发送
{
	...
	else if (B_LoRaCmd03_Master) // 读取从机参数
	{
		...
		Txd3TmpBuffer[1] = LoRaRxBuf[1];	   // 功能码			ZCL 2019.3.12 这里比较特殊，用的Txd2Buffer
		Txd3TmpBuffer[2] = LoRaRxBuf[5] * 2;   // Rcv2Buffer[5]=字数 　

		if (LoRaRxBuf[5] > 210)
			LoRaRxBuf[5] = 210; // ZCL 2019.4.26 限制大小，防止数组溢出
