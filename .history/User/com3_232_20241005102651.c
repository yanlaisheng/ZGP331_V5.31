...
uc8 DtuProgVersion[7] = "5.26";		 // 7 DTU程序版本:70		修改完程序后，要改这2个地方 YLS 2023.12.06
uc8 DtuProgMakeDate[8] = "20240925"; // 8 DTU程序生成日期:71		修改完程序后，要改这2个地方 YLS 2023.12.06
...
void u2_printf(uint8_t txd_buffer[], uint16_t send_num);
...
void Com3_config(void) // 串口通讯配置
{
	...
	USART_InitStructure.USART_BaudRate = 115200; // 2.25M/BPS   RS232芯片:230.4kbps  Pw_GprsBaudRate3
		...
		if (C_NoRcv3 > NORCVMAXMS) // && Cw_Rcv3!=1 && Rcv3Buffer[0]!=0)
		{
			...
			{							//;开始处理串口信息
				...
				if (B_DoWithData)
				{
					...
					if (F_GprsMasterNotToCom || F_GprsMasterToCom)
					{
						if (Txd2Buffer[0] == 2) // 从地址检测－接收到的上位机查询指令
						{
							...
							{							// CRC校验正确
								...
								{
									// 2023.12.15 YLS 可远程修改本身的参数，修改完成后自动保存
									{
										// 添加代码+
									}
									...
									// 添加修改ZGP331本身参数功能
									if (Lw_Com3RegAddr >= 60000) // 如果地址在60000以上，就修改ZGP331本身的参数
									{
										// 这是预置本机的 设定参数；
										...
										*p_wTarget = (m << 8) + Txd2Buffer[5];
									}
									else if (Lw_Com3RegAddr < 10000) // 如果地址在10000以下，就通过LoRa转发到下位机
									{
										// ZCL 2021.11.17 转发指令
									...
									else if (Lw_Com3RegAddr >= 10000 && Lw_Com3RegAddr < 60000) // 如果地址在[10000,60000)，就通过LoRa转发到下位机
									{
										// ZCL 2021.11.17 转发指令
										B_LoraSendWrite = 1; // ZCL 2021.11.17
					...
					// ZCL 2019.3.12  主机GPRS模式下，解析出指令，准备返回！
					if (B_Com3Cmd03 && Lw_Com3RegAddr < STM32_RPAR_ADDRESS)
					{
						...
						// p_wRead = w_DNBParLst; // 读w_GprsParLst PAR区	2019.3.14
						// p_wRead = AddressConvert(Lw_Com3RegAddr); // 2023.12.16 YLS
						if (Lw_Com3RegAddr < STM32_RPAR_ADDRESS)
						{
				...
				else if (GprsPar[ConsoleInfoTypeBase]) // 控制台信息类型,=1输出调试信息
				{
					...
					{
						Txd1Buffer[k] = Rcv3Buffer[k]; // 串口3(GPRS模块)接收到的发送到串口2
...
// Com0_RcvProcess()、main()中调用SmsAndPC_SetupInq()
...
void u1_printf(char *fmt, ...)
{
	...
	// 中断方式发送
	//	Cw_Txd2 = 0;
	//	Cw_Txd2Max = strlen((const char *)Txd2Buffer);
	//	USART_SendData(USART2, Txd2Buffer[Cw_Txd2++]);
	//	USART_ITConfig(USART2, USART_IT_TC, ENABLE); // 开始发送.
