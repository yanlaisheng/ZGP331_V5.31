...
extern u16 save_Pw_EquipmentType; // 保存的显示的画面序号
...
void Switch_Page(void); // 根据参数切换画面
/* Private functions ---------------------------------------------------------*/
...
void Switch_Page(void) // 根据参数切换画面
{
	if (save_Lw_PageNo != Lw_PageNo)
	{
		save_Lw_PageNo = Lw_PageNo;

		if (Pw_EquipmentType == 0 && Lw_PageNo != 549)
		{
			Lw_PageNo = 549;
		}
		else if (Pw_EquipmentType == 1 && Lw_PageNo != 529)
		{
			Lw_PageNo = 529;
		}
		else if (Pw_EquipmentType == 2 && Lw_PageNo != 549)
		{
			Lw_PageNo = 549;
		}
	}
