...
void Com4_SlaveSend(void) // 串口2从机发送
{
	...
	if (B_Com4Cmd03) // 读取保持寄存器
	{
		...
		// 	Lw_Com4RegAddr = Lw_Com4RegAddr - DNB_ADDRESS; // STM32_WRPAR_ADDRESS
		// }
	...
	else if (B_Com4Cmd16 || B_Com4Cmd06) // 16预置多寄存器
	{
		...
		if (j)
		{
			...
			if (Lw_Com4RegAddr < STM32_RPAR_ADDRESS)
			{
				...
				{
					Lw_Com4RegAddr = Lw_Com4RegAddr - DNB_ADDRESS; // STM32_WRPAR_ADDRESS 2023.12.16 YLS
				}
				else // 变频电机
				{
					Lw_Com4RegAddr = Lw_Com4RegAddr - STM32_WRPAR_ADDRESS;
				}
			}
			else if (Lw_Com4RegAddr < DSP_WRPAR_ADDRESS)
			{
