...
extern void Check_OTA(void); // 检查是否需要升级 YLS 2024.05.18
			...
			{							//;开始处理串口信息
				...
				case 0x1B:
					...
					if (ptr != NULL)
					{
						...
						if (len1 > 1500)
						{
							len1 = 1500; // ZCL 2019.4.26 限制大小，防止数组溢出
				...
				if (B_DoWithData)
				{
							...
							if (Txd2Buffer[0] == Pw_LoRaEquipmentNo) // 从地址检测－接收到的上位机查询指令
							{
								...
								{							// CRC校验正确
									...
									else if (Txd2Buffer[1] == 6) // 06预置单寄存器
									{
										...
										{
											// 这是预置本机的 设定参数；
											if (Lw_Com3RegAddr >= 60000 && Lw_Com3RegAddr < 61000)
											{
												p_wTarget = Pw_ParLst; // 写Pw_ParLst区
												p_wTarget += Lw_Com3RegAddr - 60000;
											}
											else if (Lw_Com3RegAddr >= 61000 && Lw_Com3RegAddr < 62000)
											{
												p_wTarget = w_GprsParLst; // 写w_GprsParLst区
												p_wTarget += Lw_Com3RegAddr - 61000;
											}
											else if (Lw_Com3RegAddr >= 62000 && Lw_Com3RegAddr < 63000)
											{
												p_wTarget = GprsPar; // 写STM32 PAR区
												p_wTarget += Lw_Com3RegAddr - 62000;
											}
											else if (Lw_Com3RegAddr >= 63000 && Lw_Com3RegAddr < 64000)
											{
												p_wTarget = Pw_ParLst_GPRS; // 写Pw_ParLst_GPRS区
												p_wTarget += Lw_Com3RegAddr - 63000;
											}
											else if (Lw_Com3RegAddr >= 64000 && Lw_Com3RegAddr < 65000)
											{
												p_wTarget = w_DNBParLst; // 写STM32 PAR区
												p_wTarget += Lw_Com3RegAddr - 64000;
											}
					...
					else if (Pw_EquipmentType == 1 || Pw_EquipmentType == 2) // 变频电机或高压电机
					{
						...
						if (B_Com3Cmd03)
						{
							...
							{
								p_wRead = w_ParLst; // 高压电机用的w_ParLst 参数区
							}
				...
				if (Pw_ConsoleInfo > 0) // 液晶屏参数：控制台信息类型,=1输出调试信息
				{
					if (Cw_BakRcv3 >= 512)
						Cw_BakRcv3 = 500; // ZCL 2019.4.26 限制大小，防止数组溢出
