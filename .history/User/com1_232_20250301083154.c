...
void Com1_RcvProcess(void)
{
	...
	if (Pw_EquipmentType == 0 || Pw_EquipmentType == 2) // 双驱泵和高压电机
	{
			...
			if (C_NoRcv1 > NORCVMAXMS) //
			{
				...
				if (Cw_BakRcv1 <= RCV1_MAX) // 接收长度正确,继续处理.
				{
					...
					)
					{
						...
						k = j >> 8;
						s = j;
						if (k == Rcv1Buffer[Cw_BakRcv1 - 2] && s == Rcv1Buffer[Cw_BakRcv1 - 1]) // CRC校验正确
						{
	...
	else if (Pw_EquipmentType == 1) // 变频电机
	{
		...
		{ // 接收处理过程
			...
			if (C_NoRcv1 > NORCVMAXMS) //
			{
				...
				if (Cw_BakRcv1 <= RCV1_MAX) // 接收长度正确,继续处理.
				{
					if (Rcv1Buffer[0] == Pw_LoRaEquipmentNo) // 从地址检测－接收到的上位机查询指令
					{
						j = CRC16(Rcv1Buffer, Cw_BakRcv1 - 2); // CRC 校验
						k = j >> 8;
						s = j;
	...
	if (Pw_EquipmentType == 0) // 双驱泵
	{
		...
		else if (B_Com1Cmd16 || B_Com1Cmd06) // 16预置多寄存器
		{
			if (B_Com1Cmd06) // 预置单个
			{
				...
				// 添加修改ZGP331本身参数功能
				if (Lw_Com1Addr >= STM32_RPAR_ADDRESS && Lw_Com1Addr < GPRS_WRPAR_ADDRESS) // 如果地址在10000以上，就修改ZGP331本身的参数
				{
					// 这是预置本机的 设定参数；
...
u16 *AddressConvert(u16 L_Reg)
{
	...
	// YLS 2025.03.01
	else if (L_Reg >= STM32_WRPAR_ADDRESS_SQB && L_Reg < GPRS_WRPAR_ADDRESS)
	{
...
// 减去偏移量的 绝对地址 YLS 2025.02.28
// 只有双驱泵程序COM3用到这个函数
u16 *AddressConvert_Com3(u16 L_Reg)
{
	u16 *p;
	...
	{
		p = Pw_ParLst; // Pw_ParLst区
		p += L_Reg - 60000;
	}
	else if (L_Reg >= 61000 && L_Reg < 62000)
	{
		p = w_ParLst; // w_ParLst区
		p += L_Reg - 61000;
	}
	else if (L_Reg >= 62000 && L_Reg < 63000)
	{
		p = w_GprsParLst; // w_GprsParLst区
		p += L_Reg - 62000;
	}
