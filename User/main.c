...
6、用串口2可以访问 w_ParLst等5区  ZCL 2019.11.21  2019.11.22 （先改的高压电机的程序，移植到这里）
	...
	#define SCR_WRPAR_ADDRESS				40000 		// 读写参数地址；w_GprsParLst，访问时偏移40000访问
...
const uint8_t testDataLong[] = {
	...
	0x82,
	0x01,
	0x01, // Extended length: 257 bytes
	0x00,
	0x01,
	0x02,
	0x03,
...
int main(void)
{
	SCB->VTOR = FLASH_BASE | FLASH_APP1_ADDR; // 设置中断向量偏移
	...
	// 重新启用全局中断
	__enable_irq();
	XmodemFileTransfer_Com1(); // Xmodem函数里传输时是57600，结束后设成正常波特率(一般9600)
	...
	NVIC_Configuration(); // 中断在这里打开，不能在前面打开
	...
	while (1)
	{
		...
		ParLimit();		   // 参数限制
		DoWith();		   // 一些数据的处理
		DowithJKData_HD(); // 对监控数据重新组合，适合和达监控协议 YLS 2025.08.22
...
void GPIO_Configuration(void)
{
	...
	// 4G模块电源引脚，GP_ON/OFF(RESET):PC0  低电平关断  (以前的GPS  GP_1PPS:PC0 秒脉冲输入)
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
	//	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
...
//	NVIC_SetVectorTable(NVIC_VectTab_FLASH, 0x0);	//YLS 2024.07.16
#endif
